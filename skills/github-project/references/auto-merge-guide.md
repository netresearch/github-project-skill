...
1. Close and reopen the pull request. It re-fires the `pull_request` and `pull_request_target` events, so workflows on those triggers run again on the same head; neither the code nor the history changes. It does not re-run checks from other apps: on #201 the DCO check, CodeQL default setup and the dynamic Copilot run did not report again, so this cannot clear an "Expected" check that comes from one of them. Workflows that trigger on `reopened` run too — on #201 the auto-approve workflow added a review — and in a repository that requests Copilot or another review on those events, a reopen can put a review in flight that must finish before merging (see "Never merge while a review is announced or in flight" in `merge-strategy.md`). The existing approval was not dismissed on #201.
2. Only if that does not help, push a new commit. It re-runs every check, and it dismisses earlier approvals where `dismiss_stale_reviews_on_push` (ruleset) or `dismiss_stale_reviews` (classic protection) is set.
