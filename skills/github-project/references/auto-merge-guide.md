...
**The PR page names the waiting check — read it before any API.** The merge box lists a required check it still waits for as "Expected — Waiting for status to be reported", and that line is the answer the API evidence above could not give. In the #201 case the API disagreed with it: the check `security / Composer Audit` had a successful run on the head, byte-identical name, the required app (`integration_id` 15368), a check suite linked to the PR and `isRequired: true` with `SUCCESS` in the rollup. Re-running the job added a second successful run and changed nothing. When the page and the API disagree like this, the page decides the merge; make GitHub evaluate the requirement again:

1. Close and reopen the pull request. It re-fires the `pull_request` events, every check reports again on the same head, and neither the code nor the history changes. On #201 this turned `BLOCKED` into `CLEAN` with the same head SHA.
2. Only if that does not help, push a new commit, which also dismisses stale approvals where `dismiss_stale_reviews` is set.

No public API exposes the merge box's list — GraphQL has no `mergeRequirements` field on `PullRequest` — so a script can only report "cause unknown"; ask the person who can see the page, or open it.
