...
1. Close and reopen the pull request. It sends the `pull_request` and `pull_request_target` events with the activity type `reopened`, so every workflow whose `types:` include it — the default types do — runs again on the same head; neither the code nor the history changes. Checks outside those workflows may not come back: on #201 the Codecov and SonarQube Cloud apps reported again, but DCO, the five GitHub Advanced Security code-scanning checks (`CodeQL`, `Opengrep OSS`, `SonarCloud`, `betterleaks`, `zizmor`) and the dynamic Copilot review did not. A reopen therefore cannot clear an "Expected" check that only such an app reports. The workflows that run again include approve and review workflows — on #201 the auto-approve job posted a second approval — and in a repository that requests Copilot or another review on these events, a reopen can put a review in flight that must finish before merging (see "Never merge while a review is announced or in flight" in `merge-strategy.md`). The existing approval was not dismissed on #201.
2. Only if that does not help, push a new commit. It triggers every workflow that runs on a push to the pull request (path filters still apply), and it dismisses earlier approvals where `dismiss_stale_reviews_on_push` (ruleset) or `dismiss_stale_reviews` (classic protection) is set.
